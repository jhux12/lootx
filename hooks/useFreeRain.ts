...
const distributeEvenly = (pot: number, participants: string[]) => {
  ...
  const totalCoins = Math.round(pot * 100);
  const baseShareCoins = Math.floor(totalCoins / participants.length);
  const remainderCoins = totalCoins - baseShareCoins * participants.length;
  const payouts = new Map<string, number>();

  participants.forEach((userId, index) => {
    const bonusCoins = index < remainderCoins ? 1 : 0;
    const amountCoins = baseShareCoins + bonusCoins;
    if (amountCoins > 0) {
  ...
  const finalizeCycle = useCallback(async (): Promise<PayoutSummary | null> => {
    ...
    try {
      const summary = await runTransaction(db, async (transaction) => {
        ...
        const payouts = distributeEvenly(state.potCoins, participants);

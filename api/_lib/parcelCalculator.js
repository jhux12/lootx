...
export function calculateShipmentParcel({ items = [], shippingProfiles = [], shippingPackages = [] }) {
  ...
  const configuredRawProfile = shippingProfiles.find((profile) => profile.slug === 'raw_card' || String(profile.name).trim().toLowerCase() === 'raw card');
  const fallbackRawProfile = { ...(configuredRawProfile ?? {}), id: configuredRawProfile?.id ?? 'default_raw_card', slug: 'raw_card', name: 'Raw Card', defaultWeightOz: 0.5, active: true };
  const profileCounts = {}; const itemDimensions = []; let itemWeightOz = 0;
  const missingProfiles = []; const missingWeights = []; const missingDimensions = []; const defaultedItemIds = [];

  for (const item of items) {
    const hasAssignedProfile = typeof item.shippingProfileId === 'string' && item.shippingProfileId.trim() !== '';
    const profile = hasAssignedProfile ? profiles.get(item.shippingProfileId) : fallbackRawProfile;
    if (!profile) { missingProfiles.push(itemId(item)); continue; }
    if (!hasAssignedProfile) defaultedItemIds.push(itemId(item));
  ...
  for (const pkg of packages) {
    ...
    const warnings = [];
    if (defaultedItemIds.length) warnings.push(`${defaultedItemIds.length} unassigned item${defaultedItemIds.length === 1 ? ' was' : 's were'} calculated as Raw Card at 0.5 oz.`);
    if (Object.keys(profileCounts).length > 1) warnings.push('Mixed items were evaluated against package weight, item-count, dimension, and explicit profile restrictions.');
    return { status: 'ready', packageId: pkg.id, packageName: pkg.name, lengthIn: Number(pkg.lengthIn), widthIn: Number(pkg.widthIn), heightIn: Number(pkg.heightIn), itemWeightOz, packagingWeightOz: Number(pkg.emptyWeightOz), bufferWeightOz, totalWeightOz, profileCounts, warnings };
  }

...
import { ArrowRight, CheckCircle2, ChevronLeft, Crown, Lock } from "lucide-react";
...
interface DailySpinPageProps {
  onBack?: () => void;
  onSpinStart: (wheelType: "regular" | "premium") => Promise<{ amount: number }>;
  onSpinClaim: (wheelType: "regular" | "premium") => Promise<{ amount: number; nextClaimAt: number }>;
  onExploreBoxes: () => void;
  canSpin: boolean;
  nextClaimAt: number;
  embedded?: boolean;
...
const DEFAULT_PREMIUM_PRIZE_AMOUNTS = [250, 500, 1000, 2500, 5000, 10000];
const PRIZE_ANGLES = [30, 90, 150, 210, 270, 330];

const getWheelPrizes = (dailySpinOdds?: Record<string, number>, fallbackAmounts = DEFAULT_PRIZE_AMOUNTS): SpinPrize[] => {
  const configuredAmounts = Object.keys(dailySpinOdds ?? {})
    .map((amount) => Math.floor(Number(amount)))
    .filter((amount) => Number.isFinite(amount) && amount > 0)
    .sort((a, b) => a - b);

  const amounts = Array.from(new Set([...configuredAmounts, ...fallbackAmounts])).slice(0, PRIZE_ANGLES.length);
...
export const DailySpinPage: React.FC<DailySpinPageProps> = ({
  ...
  const [selectedWheelType, setSelectedWheelType] = useState<"regular" | "premium">(() => hasDeposited ? "premium" : "regular");
  const [isWheelVisible, setIsWheelVisible] = useState(
    () => canSpin || nextClaimAt <= Date.now(),
  );

  const activeWheelType = hasDeposited ? "premium" : selectedWheelType;
  const isPremiumWheel = activeWheelType === "premium";
  const prizes = useMemo(() => getWheelPrizes(
    isPremiumWheel ? premiumDailySpinOdds : dailySpinOdds,
    isPremiumWheel ? DEFAULT_PREMIUM_PRIZE_AMOUNTS : DEFAULT_PRIZE_AMOUNTS
  ), [dailySpinOdds, isPremiumWheel, premiumDailySpinOdds]);
  const effectiveNextClaimAt = Math.max(localNextClaimAt, nextClaimAt);

  useEffect(() => {
    if (hasDeposited) setSelectedWheelType("premium");
  }, [hasDeposited]);
  ...
  const handleSpin = async () => {
    if (isSpinning || !canSpinNow || (activeWheelType === "premium" && !hasDeposited)) return;
    ...
    try {
      const startResult = await onSpinStart(activeWheelType);
      const winner =
      ...
      window.setTimeout(async () => {
        try {
          const claimResult = await onSpinClaim(activeWheelType);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0 bg-neutral-950">
        ...
        <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] ${isPremiumWheel ? "from-amber-500/25 via-purple-950/30 to-neutral-950" : "from-blue-900/20 via-neutral-950 to-neutral-950"}`} />
      ...
      <div className="relative z-10 w-full flex flex-col items-center max-w-6xl mx-auto px-4 py-6 md:py-8">
        ...
        {!hasDeposited && (
          <div className="mb-5 grid w-full max-w-md grid-cols-2 gap-2 rounded-2xl border border-white/10 bg-black/30 p-1.5 backdrop-blur">
            <button type="button" onClick={() => setSelectedWheelType("regular")} className={`rounded-xl px-3 py-2 text-xs font-black uppercase tracking-wide transition ${activeWheelType === "regular" ? "bg-[#205DD7] text-white shadow-lg" : "text-neutral-400 hover:text-white"}`}>Free Wheel</button>
            <button type="button" onClick={() => setSelectedWheelType("premium")} className={`flex items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-xs font-black uppercase tracking-wide transition ${activeWheelType === "premium" ? "bg-gradient-to-r from-amber-300 to-yellow-500 text-neutral-950 shadow-lg" : "text-neutral-400 hover:text-white"}`}>
        ...
        <div className="flex flex-col items-center mb-8 md:mb-12 text-center">
          <h1 className="text-4xl md:text-6xl font-black italic tracking-tighter text-white uppercase drop-shadow-2xl">
            Daily{" "}
            <span className={`text-transparent bg-clip-text bg-gradient-to-r ${isPremiumWheel ? "from-amber-200 via-yellow-400 to-fuchsia-300" : "from-[#205DD7] to-sky-300"}`}>
              {isPremiumWheel ? "Premium Spin" : "Spin"}
            </span>
          </h1>
          <p className="text-neutral-400 font-medium mt-2">
            {isPremiumWheel ? "Premium rewards unlock after your first deposit." : "Spin for free coins every 24 hours!"}
          </p>
        </div>

        {isPremiumWheel && !hasDeposited && (
          <div className="mb-5 flex w-full max-w-lg items-center gap-3 rounded-2xl border border-amber-300/25 bg-amber-300/10 p-4 text-left text-sm text-amber-100">
            <Crown className="h-5 w-5 shrink-0 text-amber-300" />
            <p><span className="font-bold text-white">Premium wheel locked.</span> Make your first deposit to unlock higher-value daily rewards. You can preview it here, but only the free wheel can spin.</p>
          </div>
        )}
          ...
          <div className="relative w-[340px] h-[340px] md:w-[510px] md:h-[510px] flex items-center justify-center max-w-full">
            ...
            >
              <path
                ...
                className={isPremiumWheel ? "fill-transparent stroke-amber-300" : "fill-transparent stroke-[#205DD7]"}
                strokeWidth="5"
              />
            </svg>
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 z-20 -mt-2 md:-mt-4 drop-shadow-xl">
              ...
              >
                <path d="M30 60L0 0H60L30 60Z" fill={isPremiumWheel ? "#facc15" : "#2f75ff"} />
                <path d="M30 50L10 10H50L30 50Z" fill={isPremiumWheel ? "#a16207" : "#4338ca"} />
              </svg>
            </div>
            ...
            >
              <div className={`relative w-full h-full overflow-hidden rounded-full border-4 border-neutral-800 shadow-2xl bg-[#1a1a1a] ${isPremiumWheel ? "border-amber-300/70 shadow-[0_0_90px_rgba(250,204,21,0.28)]" : ""}`}>
                <svg
                  viewBox="0 0 400 400"
                ...
                >
                  <path
                    ...
                    className={isPremiumWheel ? "fill-amber-300/10 stroke-amber-200/30" : "fill-white/5 stroke-neutral-800"}
                    strokeWidth="2"
                  />
                  <path
                  ...
                  >
                    <div className="relative w-full h-full flex flex-col items-center justify-center">
                      <img
                        src={prize.image}
                        alt={`${prize.amount}`}
                        className={`w-10 h-10 md:w-14 md:h-14 object-contain ${isPremiumWheel ? "drop-shadow-[0_0_18px_rgba(250,204,21,0.8)]" : "drop-shadow-[0_0_15px_rgba(32,93,215,0.5)]"} pb-4`}
                        loading="lazy"
                        decoding="async"
                        width={56}
                        height={56}
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120px] h-[120px] md:w-[175px] md:h-[175px] z-30 group">
              ...
              <button
                onClick={handleSpin}
                disabled={isSpinning || !canSpinNow || (isPremiumWheel && !hasDeposited)}
                ...
                {lastPrize ? (
                  ...
                  <>
                    ...
                    <span className="text-[10px] md:text-xs text-blue-200 mt-1 font-medium">
                      {isPremiumWheel ? "Premium Daily Reward" : "Free Daily Reward"}
                    </span>
                  </>
                )}
              </button>

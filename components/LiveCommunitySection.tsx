...
export const LiveCommunitySection: React.FC = () => {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[#11161f] p-3 sm:p-4 [touch-action:pan-y]" aria-label="Live community">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.18em] text-slate-100"><span className="h-2.5 w-2.5 rounded-full bg-emerald-300" /></h2>
      ...
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-6">
        {visiblePosts.map((post) => (
          <article key={post.id} className="overflow-hidden rounded-xl border border-violet-400/40 bg-[#070b12]">
            ...
            <img src={post.imageUrl} alt={post.caption || `${post.username} post`} className="h-24 w-full object-cover sm:h-28" loading="lazy" draggable={false} />
            <div className="p-2">
              <p className="line-clamp-1 text-[11px] text-slate-100">{post.caption || 'Community post'}</p>

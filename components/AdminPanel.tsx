...
export const AdminPanel: React.FC = () => {
  ...
  const loadLatestMarketItem = async (itemId: string) => {
      const snapshot = await getDoc(doc(db, 'items', itemId));
      if (!snapshot.exists()) return null;
      const data = snapshot.data() as Partial<CaseItem>;
      const price = Math.max(0, Number(data.valueCoins ?? data.price ?? 0) || 0);
      return {
          id: itemId,
          name: data.name ?? 'Mystery Item',
          price,
          ...
          valueUsd: data.valueUsd !== undefined ? Number(data.valueUsd) : coinsToMarketUsd(price),
          valueCoins: data.valueCoins !== undefined ? Number(data.valueCoins) : price,
          sellBackCoins: data.sellBackCoins !== undefined ? Number(data.sellBackCoins) : marketSellBackCoins(price),
  ...
  const handleCheckSelectedItemMarketPrice = (item: CaseItem) => {
      void runSelectedItemMarketAction(`check-${item.id}`, async () => {
          ...
          return `Checked ${item.name}. Updated ${result.updated ?? 0}, pending review ${result.pendingReview ?? 0}, failed ${result.failed ?? 0}. Box odds refreshed from the latest approved value.`;
      });
  };

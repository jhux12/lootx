...
export const BoxMarketPricingEditor = ({ box, items, onItemsChange }: { box: MysteryBox; items: CaseItem[]; onItemsChange: (items: CaseItem[]) => void }) => {
  ...
  const findMatch = async (item: CaseItem) => {
    ...
    try {
      ...
      const priorMatch = matches.find((entry) => entry.tcgdexId === item.tcgdexId);
      const selectedCard = priorMatch?.tcgdexId || (matches.length === 1 ? matches[0].tcgdexId : undefined);
      const selectedMatch = matches.find((entry) => entry.tcgdexId === selectedCard);
      const selectedVariant = selectedMatch?.variants.some((entry) => entry.key === item.pricingVariant)
        ? item.pricingVariant
        : selectedMatch?.variants.length === 1 ? selectedMatch.variants[0].key : undefined;
      onItemsChange(items.map((entry) => entry.id === item.id ? { ...entry, tcgplayerUrl: result.canonicalUrl, tcgplayerProductId: result.productId } : entry));
      patchState(item.id, { busy: false, matches, canonicalUrl: result.canonicalUrl, productId: result.productId, selectedCard, selectedVariant, error: matches.length ? undefined : 'Automatic price unavailable' });
  ...
  return <section className="mt-5 rounded-2xl border border-cyan-500/30 bg-[#0b0e14] p-3 sm:p-5">
    ...
    <div className="mt-5 space-y-4">{items.map((item) => { const state = states[item.id] || { url: '', matches: [] }; const match = state.matches.find((entry) => entry.tcgdexId === state.selectedCard); const savedLink = state.canonicalUrl || item.tcgplayerUrl; const safeSavedLink = /^https:\/\/www\.tcgplayer\.com\/product\/\d+(?:\/|$)/i.test(savedLink || '') ? savedLink : null; return <article key={item.id} className="rounded-xl border border-white/10 p-3 sm:p-4">
      ...
      <label className="mt-3 block text-xs font-bold text-gray-300">Paste TCGplayer Link<div className="mt-1 flex flex-col gap-2 sm:flex-row"><div className="relative flex-1"><Link2 className="absolute left-3 top-3.5 h-4 w-4 text-gray-500"/><input type="url" value={state.url} onChange={(event) => patchState(item.id, { url: event.target.value })} placeholder="https://www.tcgplayer.com/product/660414/..." className="min-h-11 w-full rounded-xl border border-gray-700 bg-[#131720] pl-10 pr-3 text-sm text-white"/></div><button type="button" disabled={state.busy || !state.url} onClick={() => findMatch(item)} className="min-h-11 rounded-xl bg-blue-600 px-4 text-sm font-bold text-white disabled:opacity-50"><Search className="mr-2 inline h-4 w-4"/>{state.busy ? 'Refreshing…' : item.tcgplayerUrl ? 'Refresh Price' : 'Find & Save Card'}</button></div></label>
      {safeSavedLink && <div className="mt-2 flex flex-col gap-2 rounded-lg border border-cyan-500/20 bg-cyan-500/5 p-2 text-xs sm:flex-row sm:items-center sm:justify-between"><span className="min-w-0 truncate text-cyan-100">Saved TCGplayer link</span><a href={safeSavedLink} target="_blank" rel="noreferrer" className="min-h-10 rounded-lg border border-cyan-400/30 px-3 py-2.5 text-center font-bold text-cyan-200">Open saved link</a></div>}

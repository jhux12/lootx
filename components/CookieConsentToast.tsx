...
export const CookieConsentToast: React.FC<Props> = ({ onAnalyticsConsent }) => {
  ...
  return <>
    {banner && <section aria-label="Cookie consent" className="fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom)+76px)] z-[70] px-3 sm:bottom-5 sm:px-5">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-[22px] border border-violet-300/20 bg-[#0b0911]/[0.97] shadow-[0_24px_90px_rgba(0,0,0,.72),0_0_45px_rgba(124,58,237,.16)] backdrop-blur-xl">
        <div className="h-px bg-gradient-to-r from-transparent via-violet-400/70 to-transparent" />
        <div className="flex flex-col gap-5 p-5 sm:p-6 lg:flex-row lg:items-center lg:gap-8"><div className="flex min-w-0 flex-1 gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-violet-300/20 bg-gradient-to-br from-violet-500/25 to-fuchsia-500/10 text-violet-200 shadow-inner"><Cookie className="h-6 w-6" /></div><div><h2 className="text-lg font-extrabold tracking-tight text-white sm:text-xl">Your privacy, your choice</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-400">We use optional cookies to understand what works and show more relevant ads. Necessary cookies keep Ripza secure and always stay on.</p><button onClick={() => setSettings(true)} className="mt-2 min-h-11 text-sm font-bold text-violet-300 underline decoration-violet-400/40 underline-offset-4 transition hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300">Manage preferences</button></div></div>

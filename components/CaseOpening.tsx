...
interface SpinnerReelCardProps {
  ...
  setCardRef: (node: HTMLDivElement | null, idx: number) => void;
}
...
export const CaseOpening: React.FC<CaseOpeningProps> = ({ boxId, isFree = false }) => {
  ...
  const reelCardRefs = useRef<(HTMLDivElement | null)[]>([]);
  ...
  const animateSpin = useCallback(async (
    ...
    applyFocusedCardStyle(reelCardRefs.current[startingCenterIndex], true);
    ...
    frameId = window.requestAnimationFrame(tick);
  }, [applyFocusedCardStyle, clampTranslate, getApproachOffset, getCenteredIndexFromTranslate, playSound, resetSpinnerAnimation, resolveCenteredTranslate, updateFocusedCard]);
  ...
  const setCardRef = useCallback((node: HTMLDivElement | null, idx: number) => {
    reelCardRefs.current[idx] = node;
  }, []);

  const applyFocusedCardStyle = useCallback((node: HTMLDivElement | null, focused: boolean) => {
    if (!node) return;
    const color = node.getAttribute('data-color') || '#ffffff';
    node.style.opacity = focused ? '1' : '0.38';
    node.style.zIndex = focused ? '4' : '1';
    node.style.boxShadow = focused ? `0 0 18px ${color}55` : 'none';
  }, []);

  const updateFocusedCard = useCallback((nextIndex: number) => {
    const prevIndex = lastCenterIndexRef.current;
    if (prevIndex === nextIndex) return;
    applyFocusedCardStyle(reelCardRefs.current[prevIndex], false);
    applyFocusedCardStyle(reelCardRefs.current[nextIndex], true);
    lastCenterIndexRef.current = nextIndex;
  }, [applyFocusedCardStyle]);

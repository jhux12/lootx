...
const addHexAlpha = (hex: string, alphaHex: string) => {
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:grid sm:grid-cols-4 sm:gap-4 sm:overflow-visible sm:px-0 lg:grid-cols-6">
        {isLoading
          ...
          : legendaryDrops.map(({ item, box }) => (
            ...
            >
              <div
                className="flex items-center justify-center rounded-xl border border-white/10 p-3"
                style={{
                  backgroundColor: addHexAlpha(item.color, '12'),
                  borderColor: addHexAlpha(item.color, '33')
                }}
              >
                <img
                  src={item.image}
              ...
              <div className="space-y-2">
                <span className="inline-flex w-fit items-center rounded-full border border-purple-300/30 bg-purple-400/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.2em] text-purple-100/80">
                  Legendary
                </span>
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-sm font-normal text-gray-300">
                    <span
                      className="h-2 w-2 rounded-full"
                      style={{ backgroundColor: addHexAlpha(item.color, 'cc') }}
                    />
                    <p>{item.name}</p>
                  </div>
                  <CoinAmount
                    ...
                    textClassName="text-gray-200 tabular-nums"
                  />

...
export const HomepageFaqSection: React.FC<{ standalone?: boolean }> = ({ standalone = false }) => {
  ...
  return <section id="faq" aria-labelledby="homepage-faq-title" className={`pullz-legacy-theme scroll-mt-24 ${standalone ? 'mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14' : 'px-3 pb-8 pt-2 sm:px-0 sm:pb-10'}`}>
    <div className="grid gap-7 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] lg:gap-12">
      <div className="lg:sticky lg:top-24 lg:self-start"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-sky-200/80">Frequently asked questions</p><h2 id="homepage-faq-title" className="mt-3 max-w-md text-3xl font-black tracking-tight text-white sm:text-4xl">Questions before your first pull?</h2><p className="mt-4 max-w-md text-sm leading-6 text-slate-400">Learn how boxes, item ownership, sell-backs, shipping, odds, and account rewards work.</p><button type="button" onClick={support} className="mt-5 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-sky-200 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/80"><ExternalLink className="h-4 w-4" />Still need help? Contact support</button></div>

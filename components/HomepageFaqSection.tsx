...
export const HomepageFaqSection: React.FC<{ standalone?: boolean }> = ({ standalone = false }) => {
  ...
  return <section id="faq" aria-labelledby="homepage-faq-title" className={`pullz-legacy-theme scroll-mt-24 ${standalone ? 'mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14' : 'px-3 pb-8 pt-2 sm:px-0 sm:pb-10'}`}>
    <div className="grid gap-7 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] lg:gap-12">
      <div className="lg:sticky lg:top-24 lg:self-start"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-sky-200/80">Frequently asked questions</p><h2 id="homepage-faq-title" className="mt-3 max-w-md text-3xl font-black tracking-tight text-white sm:text-4xl">Questions before your first pull?</h2><p className="mt-4 max-w-md text-sm leading-6 text-slate-400">Learn how packs, item ownership, sell-backs, shipping, odds, and account rewards work.</p><button type="button" onClick={support} className="mt-5 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-sky-200 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/80"><ExternalLink className="h-4 w-4" />Still need help? Contact support</button></div>
      <div className="space-y-2.5">{HOMEPAGE_FAQS.map((faq) => <FaqAccordionItem key={faq.id} faq={faq} isOpen={openId === faq.id} onToggle={() => toggle(faq.id)} />)}
        <aside className="mt-5 rounded-2xl border border-violet-400/20 bg-[radial-gradient(circle_at_100%_0%,rgba(124,92,255,0.18),transparent_46%),#111b2d] p-5 shadow-[0_12px_32px_rgba(0,0,0,0.18)]"><CheckCircle2 className="h-5 w-5 text-emerald-400" aria-hidden="true" /><h3 className="mt-3 text-xl font-black text-white">Ready to see what you pull?</h3><p className="mt-2 text-sm leading-6 text-slate-400">Browse the available packs, review the odds, and choose the one that fits your collection.</p><div className="mt-4 flex flex-col gap-2 sm:flex-row"><button type="button" onClick={() => setView({ type: 'BOXES' })} className="min-h-11 rounded-xl bg-gradient-to-r from-[#205DD7] to-sky-500 px-4 text-xs font-black uppercase tracking-wide text-[#fff] shadow-[0_8px_20px_rgba(32,93,215,0.28)] transition hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-200">View all packs</button><button type="button" onClick={support} className="min-h-11 rounded-xl border border-white/15 px-4 text-xs font-black uppercase tracking-wide text-slate-200 transition hover:border-sky-300/45 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/80">Contact support</button></div></aside>
      </div>
    </div>
  </section>;
};

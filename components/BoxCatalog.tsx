    ...
    >
      <div className="relative px-2 pb-2 pt-3">
        ...
        <div className="mx-auto aspect-[1.35] w-full">
          <BlurImage
            src={box.image}
            fallbackSrc="/home-banners/homepage-preview.svg"
            alt={box.name}
        ...
        <div className="mt-1 grid grid-cols-3 gap-1.5">
          {topItemPreviews.map((item) => (
            <div key={`${box.id}-${item.id}`} className="flex h-11 items-center justify-center rounded-md border border-white/10 bg-[#1f2730] p-1">
              <BlurImage src={item.image} fallbackSrc="/home-banners/homepage-preview.svg" alt={item.name} className="h-full w-full object-contain" loading="lazy" width={56} height={44} staticRender={staticImages} retryOnError={!staticImages} />
            </div>
          ))}
        </div>
      </div>

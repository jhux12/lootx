...
export const HomeReplicaBelowFold: React.FC<HomeReplicaBelowFoldProps> = ({ boxes, showSignupCta, onSignUp }) => {
  ...
  const [topPullOffset, setTopPullOffset] = useState(0);

  useEffect(() => {
    if (topPullCandidates.length <= 3) return;
    const intervalId = window.setInterval(() => {
      setTopPullOffset((prev) => (prev + 1) % topPullCandidates.length);
    }, 2600);
    return () => window.clearInterval(intervalId);
  }, [topPullCandidates.length]);
      ...
      <aside className="space-y-6 lg:sticky lg:top-20 lg:col-start-2 lg:row-start-1 lg:row-span-2">
        <section>
          ...
          <div className="space-y-3">
            {topPullz.map((item) => (
              <div key={item.id} className="group relative min-h-[200px] overflow-hidden rounded-xl bg-[#1f2730] p-2.5 text-center sm:min-h-[245px] sm:rounded-2xl sm:p-4">
                ...
                <div className="relative z-10 flex min-h-[160px] flex-col items-center justify-center gap-2.5 transition-transform duration-300 ease-out group-hover:translate-y-[120%] group-focus-within:translate-y-[120%] sm:min-h-[210px] sm:gap-3">
                  <div className="relative flex h-36 w-full items-center justify-center sm:h-56">
                    ...
                    <img src={item.itemImage} alt={item.itemName} className="relative z-10 max-h-28 max-w-[92%] object-contain drop-shadow-xl sm:max-h-48" loading="lazy" decoding="async" width={220} height={220} />
                  </div>
                  <p className="max-w-[150px] truncate text-xs text-slate-300 sm:max-w-[200px] sm:text-sm">{item.itemName}</p>
                  <CoinAmount amount={Math.round(item.itemPrice)} className="justify-center text-lg font-black text-white sm:text-xl" iconClassName="h-4 w-4 sm:h-5 sm:w-5" />
                </div>
                <div className="absolute inset-0 z-20 grid translate-y-[125%] place-items-center opacity-0 transition-all duration-300 ease-out group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100">
                  <img src={item.boxImage} alt={item.boxName} className="h-24 w-24 rounded-lg object-cover sm:h-36 sm:w-36 sm:rounded-xl" loading="lazy" decoding="async" width={144} height={144} />
                </div>
              </div>
            ))}
          </div>

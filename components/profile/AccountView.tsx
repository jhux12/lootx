      ...
      <section className="max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-t-[1.75rem] border border-white/10 bg-[#15151b] px-5 pb-7 pt-3 shadow-2xl sm:max-h-[90dvh] sm:rounded-[1.75rem] sm:px-7">
        ...
        <div className="mt-6 space-y-7">
          ...
          {activePanel === 'settings' && <fieldset>
            ...
            {validationResult?.status === 'invalid' && <div className="mt-4 rounded-xl border border-red-400/30 bg-red-400/10 p-4 text-sm text-red-100"><strong>We couldn't verify this address.</strong><ul className="mt-1 list-disc pl-5">{(validationResult.messages?.length ? validationResult.messages : ['Please review the highlighted address details.']).map((message) => <li key={message}>{message}</li>)}</ul>{validationResult.attemptId && <><p className="mt-3 text-red-100/80">If you're sure it is correct, you can save it now and update it before requesting shipment.</p><button type="button" onClick={() => onAddressChoice('original')} className="mt-3 min-h-11 w-full rounded-xl border border-red-200/30 bg-red-200/10 px-3 font-black text-white transition hover:bg-red-200/20 sm:w-auto">Save Anyway</button></>}</div>}
          </fieldset>}

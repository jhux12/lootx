        ...
        <section className="rounded-2xl border border-white/10 bg-[#1f252c] p-4">
          ...
          <div className="grid grid-cols-1 gap-2">
            {([
              ...
              <input key={key} value={addressForm[key]} onChange={(event) => setAddressForm({ ...addressForm, [key]: event.target.value })} placeholder={label} className="rounded-xl border border-white/10 bg-[#2a323b] px-3 py-2 text-sm text-white" />
            ))}
          </div>

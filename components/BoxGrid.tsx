...
export const BoxGrid: React.FC = () => {
  ...
  return (
    <section id="popular-boxes" className="mt-14 px-4 md:px-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <h2 className="text-2xl font-semibold text-white flex items-center gap-2">
          <Boxes className="w-5 h-5 text-cyan-400" /> Popular boxes
        </h2>
        <button
          className="w-full sm:w-auto rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-gray-200 transition-colors hover:border-white/25 hover:text-white"
          onClick={() => {
            playSound('click');
            setView({ type: 'BOXES' });
          }}
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {canViewMore && (
          <button
            className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-gray-200 transition-colors hover:border-white/25 hover:text-white"
            onClick={() => {
              playSound('click');

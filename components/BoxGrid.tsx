...
export const BoxGrid: React.FC = () => {
  ...
  return (
    <section id="popular-boxes" className="mt-16">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-2 text-white">
          <Boxes className="w-5 h-5 text-sky-400" />
          <h2 className="text-xl font-semibold">Popular boxes</h2>
        </div>
        <button
          className="w-full sm:w-auto px-4 py-2 rounded-full border border-white/10 bg-brand-card/70 text-sm font-semibold text-gray-200 transition-colors hover:border-white/25 hover:text-white"
          onClick={() => {
            playSound('click');
            setView({ type: 'BOXES' });
          }}
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {canViewMore && (
          <button
            className="px-4 py-2 rounded-full border border-white/10 bg-brand-card/70 text-sm font-semibold text-gray-200 transition-colors hover:border-white/25 hover:text-white"
            onClick={() => {
              playSound('click');

    ...
    <>
      <style>{`
        @keyframes loot-reveal {
          ...
          40% { transform: scale(1.5) rotate(0deg) translateY(-6px); filter: brightness(2) hue-rotate(25deg); }
          50% { transform: scale(1.35) rotate(1deg) translateY(-8px); filter: brightness(1.8) hue-rotate(35deg); }
          70% { transform: scale(1.05) rotate(0deg) translateY(-2px); filter: brightness(1.1) hue-rotate(10deg); }
          100% { transform: scale(1) rotate(0deg) translateY(0); filter: brightness(1) hue-rotate(0deg); }
        }
      ...
      <header className="flex items-center justify-between px-4 md:px-6 py-3 bg-[#080b14] border-b border-gray-800/50 sticky top-0 z-50">
        <div className="flex items-center gap-4 lg:gap-10">
          ...
          >
            ...
            <span
              className={`text-xl md:text-3xl font-black tracking-tight leading-none not-italic bg-gradient-to-r from-[#8b5cf6] to-[#22d3ee] text-transparent bg-clip-text inline-block origin-left ${lootRevealActive ? 'animate-loot-reveal' : ''}`}
            >
              .GG
            </span>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-8">
            <button onClick={() => handleNav({ type: 'HOME' })} className="flex items-center gap-2 text-white hover:text-blue-400 transition-colors text-sm font-semibold">
              <PackageOpen className="w-4 h-4" /> Cases
            </button>
            <button onClick={() => handleNav({ type: 'BATTLES' })} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm font-medium">
              <Swords className="w-4 h-4" /> Battles
            </button>
            <button onClick={() => handleNav({ type: 'LEADERBOARD' })} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm font-medium">
              <Trophy className="w-4 h-4" /> Leaderboard
            </button>
            <button onClick={() => handleNav({ type: 'BONUSES' })} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm font-medium">
            ...
            {user.isAdmin && (
              <button onClick={() => handleNav({ type: 'ADMIN' })} className="flex items-center gap-2 text-red-500 hover:text-red-400 transition-colors text-sm font-bold ml-4 bg-red-500/10 px-3 py-1 rounded-full border border-red-500/20">
        ...
        {isMobileMenuOpen && (
          <div className="fixed inset-0 top-[60px] bg-[#0b0e14] z-40 lg:hidden flex flex-col p-4 animate-in slide-in-from-left-full duration-200">
             <nav className="flex flex-col gap-2">
               <button onClick={() => { handleNav({ type: 'HOME' }); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-[#131720] rounded-lg text-white font-medium">
                 <PackageOpen className="w-5 h-5 text-blue-500" /> Cases
               </button>
               <button onClick={() => { handleNav({ type: 'BATTLES' }); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-[#131720] rounded-lg text-gray-300 font-medium">
                 <Swords className="w-5 h-5 text-purple-500" /> Battles
               </button>
               <button onClick={() => { handleNav({ type: 'LEADERBOARD' }); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-[#131720] rounded-lg text-gray-300 font-medium">
                 <Trophy className="w-5 h-5 text-yellow-500" /> Leaderboard
               </button>
               <button onClick={() => { handleNav({ type: 'BONUSES' }); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-[#131720] rounded-lg text-gray-300 font-medium">
               ...
               {user.isAdmin && (
                  <button onClick={() => { handleNav({ type: 'ADMIN' }); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-[#131720] rounded-lg text-red-400 font-medium">
               ...
               {isAuthenticated ? (
                   <>
                      <button onClick={() => { handleNav({ type: 'PROFILE' }); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-[#131720] rounded-lg text-gray-300 font-medium">
                          <User className="w-5 h-5 text-gray-400" /> Profile
                      </button>
                      <button onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-[#131720] rounded-lg text-red-400 font-medium">
                          <LogOut className="w-5 h-5" /> Sign out
                      </button>
                   </>
               ) : (
                  <button onClick={() => { setShowLoginModal(true); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 bg-blue-600 rounded-lg text-white font-medium justify-center">
                      Sign In
                  </button>
               )}
             </nav>

...
const drawerCardClass =
  'flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.055] p-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] transition-all duration-200 hover:-translate-y-0.5 hover:border-cyan-300/25 hover:bg-white/[0.085]';

const navPillClass =
  'flex h-10 items-center gap-2 rounded-full border border-transparent px-3.5 text-sm font-semibold text-slate-200 transition-all duration-200 hover:border-white/10 hover:bg-white/[0.075] hover:text-white hover:shadow-[0_10px_30px_rgba(0,0,0,0.22)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70';

const dropdownPanelClass =
  'absolute left-0 top-full mt-3 w-56 rounded-2xl border border-white/10 bg-[#101820]/95 p-2 shadow-[0_24px_80px_rgba(0,0,0,0.48),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl transition-all duration-200';

const dropdownItemClass =
  'flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-slate-200 transition-all duration-200 hover:bg-white/[0.07] hover:text-white';

const utilityButtonClass =
  'relative flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.045] text-slate-300 transition-all duration-200 hover:border-cyan-300/30 hover:bg-white/[0.08] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70';
...
const HeaderComponent: React.FC<HeaderProps> = ({ onOpenInbox: _onOpenInbox, unreadChatCount: _unreadChatCount, isSticky = true }) => {
  ...
  const startPullingButton = useMemo(() => (
    <button
      ...
      className="group relative inline-flex shrink-0 overflow-hidden rounded-full border border-cyan-300/20 bg-gradient-to-r from-cyan-300/15 via-fuchsia-400/15 to-amber-300/15 p-px shadow-[0_12px_34px_rgba(32,93,215,0.18)] outline-none transition-transform duration-200 hover:scale-[1.02] focus-visible:ring-2 focus-visible:ring-white/80 focus-visible:ring-offset-2 focus-visible:ring-offset-black active:scale-[0.99]"
      aria-label="Start Pulling"
    >
      <span
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(115deg,rgba(45,212,191,0.20),rgba(217,70,239,0.12),rgba(251,146,60,0.16))]"
      />
      <span className="relative z-10 inline-flex items-center justify-center rounded-full bg-[#070b12] px-4 py-2 text-sm font-extrabold leading-none tracking-tight text-white sm:px-5 sm:py-2.5 sm:text-base lg:px-4 lg:py-2.5 lg:text-sm xl:text-sm">
        <span className="whitespace-nowrap">Start Pulling</span>
      </span>
    </button>
  ), [openAuthModal, playSound]);

  const dailySpinDesktopClass = `group relative flex w-full items-center gap-3 overflow-hidden rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-slate-200 transition-all duration-200 ${
    showDailySpinReady
      ? 'border border-blue-400/45 bg-blue-500/10 text-blue-100 shadow-[0_0_18px_rgba(59,130,246,0.24)] hover:bg-blue-500/18'
  ...
  const rewardsDesktopTabClass = `group relative flex h-10 items-center gap-2 overflow-hidden rounded-full border px-3.5 text-sm font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70 ${
    showDailySpinReady
      ? 'border-blue-400/45 bg-blue-500/10 text-blue-100 shadow-[0_0_18px_rgba(59,130,246,0.24)] hover:bg-blue-500/18'
      : 'border-transparent text-slate-200 hover:border-white/10 hover:bg-white/[0.075] hover:text-white'
  }`;
  ...
  return (
    ...
    <div className="relative z-[140]">
      <header
        ref={headerRef}
        className={`${isSticky ? 'fixed inset-x-0 top-0' : 'relative'} z-[120] border-b border-white/10 bg-[#070b12]/72 shadow-[0_18px_70px_rgba(0,0,0,0.26)] backdrop-blur-2xl before:pointer-events-none before:absolute before:inset-x-0 before:bottom-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-cyan-300/35 before:to-transparent`}
      >
        <div className="pt-[env(safe-area-inset-top,0px)]">
          <nav className="mx-auto flex h-[64px] max-w-7xl items-center justify-between px-4 sm:h-[68px] lg:px-8">
            <div className="flex min-w-0 items-center gap-3 lg:gap-x-7">
            <button
              type="button"
            ...
            <div className="hidden items-center rounded-full border border-white/10 bg-white/[0.035] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] lg:flex lg:gap-x-1">
              <div
                ref={gamesMenuRef}
                className="relative"
              >
                ...
                >
                  <button type="button" onClick={() => navigate('BOXES')} className={dropdownItemClass}>
              ...
              <div ref={rewardsMenuRef} className="relative">
                ...
                <div className={`${dropdownPanelClass} ${isRewardsMenuOpen ? 'visible opacity-100 translate-y-0' : 'invisible opacity-0 -translate-y-1'}`}>
              ...
              <button onClick={() => navigate('LEADERBOARD')} className={navPillClass}>
          ...
          <div className="flex min-h-[40px] items-center gap-2 sm:gap-3">
            {!authInitialized ? (
              ...
              <>
                <div className="hidden items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur lg:flex">
                  <div className="flex h-9 min-w-[76px] items-center gap-1.5 rounded-full border border-amber-300/15 bg-black/20 px-3 py-1.5">
                  ...
                  <div className={`relative min-w-[132px] shrink-0 overflow-hidden rounded-full border border-cyan-300/15 bg-gradient-to-r from-cyan-300/10 via-fuchsia-400/10 to-amber-300/10 p-px ${balanceTone === 'up' ? 'shadow-[0_0_18px_rgba(34,197,94,0.22)]' : balanceTone === 'down' ? 'shadow-[0_0_18px_rgba(239,68,68,0.22)]' : ''}`}>
                    <span
                      aria-hidden="true"
                      className="pointer-events-none absolute inset-0 bg-[linear-gradient(115deg,rgba(45,212,191,0.20),rgba(217,70,239,0.12),rgba(251,146,60,0.16))]"
                    />
                    <div className="relative z-10 flex items-center justify-between gap-2 rounded-full bg-[#070b12]/95 py-1.5 pl-3.5 pr-1.5">
                ...
                <div className="hidden items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] lg:flex">
                  <button
                    type="button"
                ...
                <div className="hidden items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] p-1 pl-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] lg:flex">
                  {user.isAdmin && (
                    <button onClick={() => navigate('ADMIN')} className="hidden h-9 items-center gap-2 rounded-full border border-blue-400/20 bg-blue-400/10 px-3 text-xs font-bold uppercase tracking-wide text-blue-200 transition-all duration-200 hover:border-blue-300/40 hover:bg-[#205DD7] hover:text-white xl:flex">
                      <ShieldCheck className="h-3.5 w-3.5" />
                      Admin
                    </button>
                  )}
                  <button onClick={() => navigate('PROFILE')} className="min-w-[96px] max-w-[136px] rounded-full px-2 text-right transition-colors hover:bg-white/[0.05] xl:min-w-[120px]">
                    <span className="block truncate text-sm font-bold text-white hover:text-cyan-200">{resolvedDisplayName}</span>
                  </button>
                  <div className="relative flex items-center">
                    <button type="button" onClick={() => navigate('PROFILE')} className="h-9 w-9 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70">
                      <UserAvatar user={user} className="h-9 w-9 rounded-full object-cover" initialsClassName="text-xs" />
                    </button>
                    {showFreeBoxTooltip ? (
            ...
            {authInitialized && isAuthenticated && (
              <div className="flex min-h-[40px] items-center gap-1.5 lg:hidden">
                <div className="flex h-8 min-w-[62px] items-center gap-1.5 rounded-full border border-amber-300/15 bg-white/[0.045] px-2.5 py-1.5 sm:min-w-[74px] sm:px-3">
                ...
                <div className="relative min-w-[112px] shrink-0 overflow-hidden rounded-full border border-cyan-300/15 bg-gradient-to-r from-cyan-300/10 via-fuchsia-400/10 to-amber-300/10 p-px sm:min-w-[126px]">
                  <span
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-0 bg-[linear-gradient(115deg,rgba(45,212,191,0.20),rgba(217,70,239,0.12),rgba(251,146,60,0.16))]"
                  />
                  <div className="relative z-10 flex items-center justify-between gap-1.5 rounded-full bg-[#070b12]/95 py-1 pl-2.5 pr-1 sm:gap-2 sm:py-1.5 sm:pl-3.5 sm:pr-1.5">
      ...
      <div
        className={`fixed inset-x-0 bottom-[var(--pullz-mobile-bottom-nav-height,72px)] z-50 w-full overflow-y-auto overscroll-contain bg-[#0d141c]/98 px-4 pb-4 pt-3 shadow-[0_-20px_80px_rgba(0,0,0,0.45)] backdrop-blur-2xl transition-all duration-300 ease-out lg:hidden ${
          isSticky ? 'top-[var(--pullz-header-height)]' : 'top-0'
        } ${
      ...
      >
        <div className="sticky top-0 z-10 -mx-4 mb-4 flex items-center justify-between border-b border-white/10 bg-[#0d141c]/95 px-4 pb-3 pt-1 backdrop-blur">
        ...
        <div className="flex flex-col gap-6 pb-4">
          {isAuthenticated ? (
            <div className="grid grid-cols-2 gap-3">
              <>
                <button onClick={() => navigate('PROFILE')} className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#205DD7] to-cyan-500/80 py-3.5 font-bold text-white shadow-[0_16px_36px_rgba(32,93,215,0.24)]"><UserIcon className="h-5 w-5" />Account</button>
          ...
          {user.isAdmin && (
            <button onClick={() => navigate('ADMIN')} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-blue-400/25 bg-blue-500/10 py-3 font-bold text-blue-200">
          ...
          <section className="space-y-3">
            <button type="button" onClick={() => toggleMobileSection('games')} className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-white/[0.045] px-3 py-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
          ...
          <section className="space-y-3">
            <button type="button" onClick={() => toggleMobileSection('learn')} className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-white/[0.045] px-3 py-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
          ...
          <section className="space-y-3">
            <button type="button" onClick={() => toggleMobileSection('support')} className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-white/[0.045] px-3 py-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
          ...
          <section className="mt-2 flex flex-col items-center gap-6">
            <button type="button" onClick={() => toggleMobileSection('social')} className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-white/[0.045] px-3 py-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">

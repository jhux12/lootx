...
import { getTargetTranslate, SPINNER_ITEM_WIDTH } from './positioning';
...
export const PremiumCaseSpinner: React.FC<PremiumCaseSpinnerProps> = ({
  ...
  const [metrics, setMetrics] = React.useState({ itemWidth: SPINNER_ITEM_WIDTH, gap: 0, viewportWidth: 0 });
  ...
  const measureMetrics = React.useCallback(() => {
    ...
    const itemWidth = first ? first.getBoundingClientRect().width : SPINNER_ITEM_WIDTH;
    const gap = first && second
      ? second.getBoundingClientRect().left - first.getBoundingClientRect().left - itemWidth
      : 0;
    setMetrics({ itemWidth, gap: Number.isFinite(gap) ? gap : 0, viewportWidth });
  }, []);

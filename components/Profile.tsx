...
import { Check, ChevronDown, Clock, Coins, Copy, CreditCard, ExternalLink, Filter, Info, Package, PackageCheck, Plus, Search, ShieldCheck, Truck, X } from 'lucide-react';
...
const OrdersView: React.FC<{ orders: OrderSummary[] }> = ({ orders }) => {
  ...
  }, [orderSearch, orders, statusFilter]);
      ...
      <div className="grid grid-cols-1 gap-3 min-[520px]:grid-cols-3">
        <div className="rounded-2xl border border-purple-400/20 bg-[#111421]/90 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_18px_35px_rgba(0,0,0,0.22)]">
          <div className="flex items-center gap-4">
            ...
            <div><p className="text-2xl font-black leading-none">{orders.length}</p><p className="mt-1 text-sm font-medium text-gray-400">Total Orders</p></div>
          </div>
        </div>
            ...
            return (
              <article key={order.id} className="rounded-3xl border border-white/7 bg-[#101722] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] sm:p-4">
                <div className="grid gap-4 sm:grid-cols-[9rem_minmax(0,1fr)_13rem] sm:items-stretch">
                  <div className={`flex aspect-square items-center justify-center overflow-hidden rounded-2xl border bg-[#090e16] p-3 ${order.rarity === 'legendary' || order.rarity === 'epic' ? 'border-purple-400/45 shadow-[0_0_24px_rgba(168,85,247,0.22)]' : 'border-white/12'}`}>
                  ...
                  <div className="min-w-0 py-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-md border px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${order.status === 'shipped' ? 'border-emerald-400/35 bg-emerald-500/10 text-emerald-200' : 'border-amber-300/35 bg-amber-400/10 text-amber-200'}`}>{order.status}</span>
                    </div>
                    <h3 className="mt-3 line-clamp-2 text-lg font-black leading-tight tracking-[-0.03em] text-white sm:text-xl">{order.name}</h3>
                    <p className="mt-2 text-sm font-semibold text-gray-500">Order #{order.id.slice(0, 8)}</p>
                  ...
                  <div className="border-t border-white/8 pt-2 sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0">
                    ...
                    {order.trackingNumber ? <button type="button" onClick={() => { void handleCopyTracking(order.id, order.trackingNumber as string); }} className="mt-4 inline-flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-xs font-bold text-blue-100 hover:bg-white/5">{copiedTrackingId === order.id ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />} Tracking</button> : null}
                  </div>
                </div>
              </article>
            );

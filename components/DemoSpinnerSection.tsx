...
export const DemoSpinnerSection: React.FC<DemoSpinnerSectionProps> = ({ box }) => {
  ...
  const spin = useCallback(() => {
    ...
    window.setTimeout(() => {
      ...
      const finalTranslate = -winnerLeft;
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[#090d15] p-3 sm:p-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/40 py-3">
        ...
        {reelItems.length > 0 ? (
          <div ref={reelRef} className="ml-[-66px] flex gap-2 px-[50%] pb-1 pt-2 will-change-transform">
            {reelItems.map((item, index) => {
              const isLandedSlot = index === BUFFER_COUNT;
              return (
                <div
                  key={`${item.id}-${index}`}
                  className={`relative flex h-[184px] min-w-[132px] flex-col items-center justify-between overflow-hidden rounded-xl border bg-[#151a23] px-2 py-3 ${isLandedSlot ? 'ring-1 ring-white/30' : ''}`}
                  style={{
                    borderColor: `${item.color}66`,
                    boxShadow: isLandedSlot
                      ? `0 0 0 1px ${item.color}80, 0 0 20px ${item.color}55`
                      : `0 0 0 1px ${item.color}30`
                  }}
                >
                  <div
                    className="pointer-events-none absolute inset-3 rounded-full opacity-90"
                    style={{
                      background: `radial-gradient(circle, ${item.color}70 0%, ${item.color}22 45%, ${item.color}00 78%)`
                    }}
                  />
                  <img src={item.image} alt={item.name} className="relative z-10 h-20 w-20 object-contain" loading="lazy" />
                  <CoinAmount
                    amount={toCoins(item.price, PRICE_UNIT_MODE)}
                    className="relative z-10 justify-center text-sm font-bold text-gray-100"

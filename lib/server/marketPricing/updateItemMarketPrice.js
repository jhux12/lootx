...
export async function updateItemMarketPrice(itemDoc) {
  ...
  try {
    ...
    const shouldAutoApprove = !locked && !isLargeChange;
...
export async function updateEnabledMarketPrices({ itemId } = {}) {
  const docs = [];
  if (itemId) {
    const snap = await db.collection('items').doc(String(itemId)).get();

...
const marketValueForLatestItem = (latestItem, fallbackPrize) => {
  const pricing = latestItem?.marketPricing || {};
  ...
  const valueCoins = hasSuggested
    ? suggestedCoins
    : hasApproved
      ? approvedCoins
      : valueCoinsForPrize(fallbackPrize, latestItem);
  const valueUsd = hasSuggested && pricing.suggestedValueUsd != null
    ? Number(pricing.suggestedValueUsd)
    : hasApproved && pricing.approvedValueUsd != null
      ? Number(pricing.approvedValueUsd)
      : latestItem?.valueUsd ?? Math.round((valueCoins / 100) * 100) / 100;
  const sellBackCoins = hasSuggested && pricing.suggestedSellBackCoins != null
    ? Number(pricing.suggestedSellBackCoins)
    : hasApproved && pricing.approvedSellBackCoins != null
      ? Number(pricing.approvedSellBackCoins)
      : latestItem?.sellBackCoins ?? Math.floor(valueCoins * 0.8);
...
const refreshPrizeFromCatalog = (prize, latestItem) => {
  ...
  const marketValue = marketValueForLatestItem(latestItem, prize);
  return {
    ...prize,

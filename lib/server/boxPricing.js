...
const idFrom = (node) => node?.productId ?? node?.product_id ?? node?.tcgplayerProductId ?? node?.tcgplayerId;

export const extractTcgplayerVariants = (card, requiredProductId) => {
  const found = new Map();
  const visit = (node, hint = '', inheritedId = null) => {
    if (!node || typeof node !== 'object') return;
    if (Array.isArray(node)) { node.forEach((entry) => visit(entry, entry?.variant || entry?.type || entry?.name || hint, inheritedId)); return; }
...
export const findTcgplayerMatches = async ({ itemName, tcgplayerUrl, language = 'en', fetchImpl = fetch }) => {
  ...
  const cleanedName = name
    ...
    .replace(/\b(?:graded|slab|near mint|nm|raw)\b/gi, ' ')
    .replace(/\([^)]*\)|\[[^\]]*\]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  const words = cleanedName.split(/\s+/).filter(Boolean);
  const searchNames = [...new Set([cleanedName, words.slice(0, 4).join(' '), words.slice(-4).join(' '), words.slice(0, 2).join(' '), words.slice(-2).join(' '), words[0], words.at(-1)].filter(Boolean))];
  const summaries = [];
  let successfulSearches = 0;
  for (const searchName of searchNames) {
    for (const filter of [`like:${searchName}`, searchName]) {
      try {
        const result = await cachedJson(`https://api.tcgdex.net/v2/${encodeURIComponent(language)}/cards?name=${encodeURIComponent(filter)}`, fetchImpl);
        successfulSearches += 1;
        const rows = Array.isArray(result) ? result : Array.isArray(result?.data) ? result.data : Array.isArray(result?.cards) ? result.cards : [];
        for (const row of rows.slice(0, 25)) if (row?.id && !summaries.some((entry) => entry.id === row.id)) summaries.push(row);
      } catch { /* A supported filter can differ between TCGdex deployments; try the next safe form. */ }
    }
  }
  if (!successfulSearches) throw new Error('TCGDEX_SEARCH_FAILED');
  const list = summaries.slice(0, 100);
  const cardResults = await Promise.allSettled(list.filter((entry) => entry?.id).map((entry) => cachedJson(`https://api.tcgdex.net/v2/${encodeURIComponent(language)}/cards/${encodeURIComponent(entry.id)}`, fetchImpl)));
  const cards = cardResults.filter((entry) => entry.status === 'fulfilled').map((entry) => entry.value);
  return {
    ...parsed,
    searchedNames: searchNames,
    matches: cards.filter((card) => containsProductId(card?.pricing, parsed.productId) || containsProductId(card?.variants_detailed, parsed.productId) || containsProductId(card?.variantsDetailed, parsed.productId)).map((card) => ({
...
export const buildLinkedItems = (items, itemId, parsedUrl) => (items || []).map((item) => item.id === itemId
  ? withoutUndefined({ ...item, tcgplayerUrl: parsedUrl.canonicalUrl, tcgplayerProductId: parsedUrl.productId })
  : item);

...
import { Box, Check, ChevronDown, Coins, Filter, ListTree, PackageOpen, Search, ShieldCheck, X } from 'lucide-react';
...
export default function UpgraderPage() {
  const { boxes, isAuthenticated, openAuthModal, addInventoryItemFromServer } = useGame();
  ...
  const openDropTable = () => {
    ...
    trackEvent('build_box_drop_table_view', { item_count: selected.length, price: quote.price });
...
function Summary({selectedItems,quote,quoteError,minOk,varietyOk,onRemove,onReview,onDropTable,canReview,compact,open,setOpen}:any){return <div className="sticky top-4 rounded-2xl border border-white/15 bg-[#0c121b] shadow-[0_0_34px_rgba(0,0,0,.55)]"><div className="flex items-center justify-between p-4"><div><p className="text-lg font-black">{selectedItems.length} items selected</p>{!minOk?<p className="text-xs text-amber-300">Choose at least 5 possible pulls to build your box.</p>:!varietyOk?<p className="text-xs text-amber-300">Choose at least two values or rarity tiers.</p>:<p className="text-xs text-emerald-300">Custom pool ready</p>}</div>{compact&&<button onClick={()=>setOpen?.(!open)}><ChevronDown className={open?'rotate-180':''}/></button>}</div>{(!compact||open)&&<div className="border-t border-white/10 p-4"><div className="no-scrollbar flex gap-2 overflow-x-auto pb-3">{selectedItems.map((item:any)=><div key={item.id} className="relative shrink-0"><img src={item.image} className="h-14 w-12 rounded-md border border-white/10 object-contain bg-black/30"/><button onClick={()=>onRemove(item.id)} className="absolute -right-2 -top-2 rounded-full bg-black p-0.5"><X className="h-3 w-3"/></button></div>)}</div><div className="flex items-end justify-between"><div><p className="text-sm font-bold">Box Total</p><p className="text-4xl font-black"><Coins className="mr-1 inline h-8 w-8 text-yellow-300"/>{(quote?.price??0).toLocaleString()}</p>{quote?.savings>0?<p className="text-sm text-green-400">You save {quote.savings.toLocaleString()} coins!</p>:null}</div><ShieldCheck className="h-5 w-5 text-sky-300"/></div>{quoteError&&<p className="mt-2 text-xs text-red-300">{quoteError}</p>}</div>}<div className="m-3 grid grid-cols-[minmax(0,1fr)_4.5rem] gap-2"><button onClick={onReview} disabled={!canReview} className="flex h-14 min-w-0 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-violet-600 to-sky-400 px-3 text-base font-black disabled:opacity-45 sm:text-lg"><Box className="h-5 w-5 shrink-0"/><span className="truncate">Build & Open Box</span><span className="hidden sm:inline">— {(quote?.price??0).toLocaleString()} Coins</span></button><button onClick={onDropTable} disabled={!canReview} className="grid h-14 place-items-center rounded-2xl border border-violet-400/50 bg-violet-500/15 text-violet-100 shadow-[0_0_18px_rgba(139,92,246,.22)] disabled:opacity-45" aria-label="View drop table odds"><span className="flex flex-col items-center text-[10px] font-black uppercase leading-none"><ListTree className="mb-0.5 h-5 w-5"/>Odds</span></button></div></div>}
function DropTableModal({quote,onClose}:{quote:Quote;onClose:()=>void}){const shown=quote.items.slice(0,6);const remaining=quote.items.slice(6);const remainingChance=remaining.reduce((sum,item)=>sum+Number(item.chance??0),0);return <div className="fixed inset-0 z-[310] flex items-end bg-black/75 p-0 sm:items-center sm:justify-center sm:p-4"><div className="max-h-[100dvh] w-full overflow-y-auto rounded-t-[2rem] border border-white/15 bg-[#080d18] p-5 text-white shadow-2xl sm:max-w-md sm:rounded-[2rem]"><div className="mb-4 flex items-center justify-between"><button onClick={onClose} className="rounded-full p-2 text-slate-300 hover:bg-white/10" aria-label="Close drop table"><X className="h-5 w-5"/></button><h2 className="text-lg font-black tracking-wide">BUILD A BOX</h2><span className="w-9"/></div><div className="relative mb-5 grid h-44 place-items-center overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_50%_35%,rgba(124,58,237,.55),transparent_38%),linear-gradient(135deg,#101729,#050812)]"><PackageOpen className="h-28 w-28 text-violet-300 drop-shadow-[0_0_28px_rgba(139,92,246,.75)]"/></div><div className="text-center"><h3 className="bg-gradient-to-r from-violet-300 to-fuchsia-500 bg-clip-text text-3xl font-black text-transparent">ODDS PREVIEW</h3><p className="mx-auto mt-3 max-w-xs text-sm leading-6 text-slate-300">Odds are calculated securely on the server from your selected pull pool.</p></div><div className="mt-5 rounded-3xl border border-white/10 bg-black/25 p-4"><div className="mb-4 rounded-2xl border border-white/10 bg-white/[.05] p-4 text-center"><p className="text-xs text-slate-400">Box Cost</p><p className="mt-1 text-2xl font-black"><Coins className="mr-1 inline h-5 w-5 text-yellow-300"/>{quote.price.toLocaleString()}</p></div><div className="space-y-4">{shown.map((item)=><div key={item.id} className="grid grid-cols-[4.25rem_minmax(0,1fr)_4rem] gap-3"><img src={item.image} alt="" className="h-20 w-16 rounded-lg bg-black/40 object-contain"/><div className="min-w-0"><p className="truncate font-bold">{item.name}</p><p className="truncate text-xs text-slate-400">{item.rarity}</p><span className="mt-1 inline-flex max-w-full rounded-md bg-violet-600/30 px-2 py-1 text-xs text-violet-100">From {item.primaryBoxName}</span><div className="mt-3 h-1.5 rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-500" style={{width:`${Math.min(100,Math.max(2,item.chance*8))}%`}} /></div></div><p className="text-right text-xl font-black">{item.chance.toFixed(2)}%</p></div>)}{remaining.length>0&&<div className="flex items-center justify-between pt-1 text-sm text-slate-200"><span>＋ {remaining.length} more items</span><span className="text-lg font-black">{remainingChance.toFixed(2)}%</span></div>}</div></div><div className="mt-3 flex items-center justify-between rounded-2xl border border-white/10 bg-black/25 p-4 text-xl font-black"><span>Total</span><span>100.00%</span></div><button onClick={onClose} className="mt-5 h-14 w-full rounded-2xl bg-gradient-to-r from-violet-700 to-fuchsia-600 text-lg font-black">CLOSE</button></div></div>}

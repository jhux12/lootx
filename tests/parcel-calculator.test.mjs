...
test('unassigned items default to Raw Card at 0.5 oz', () => {
  const result = calculate([{ id: 'legacy-unassigned' }]);
  assert.equal(result.status, 'ready');
  assert.equal(result.itemWeightOz, 0.5);
  assert.equal(result.totalWeightOz, 3.5);
  assert.deepEqual(result.profileCounts, { raw: 1 });
  assert.match(result.warnings[0], /Raw Card at 0\.5 oz/);
});
